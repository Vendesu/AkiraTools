...
instal_otomasi_telegram() {
    ...
    sudo wget -q "$URL_AKIRATOOLS" -O /usr/bin/akiratools.py
    sudo wget -q "$URL_GRUP" -O /usr/bin/grup.py
    sudo wget -q "$URL_SPAMUP" -O /usr/bin/spamup.py
    sudo wget -q "$URL_SPAMORI" -O /usr/bin/spamori.py
    animasi_ketik "Oke, semua file udah kedownload!" "${HIJAU}"

    echo -e "\n${CYAN}[4/4]${RESET} Tinggal setting dikit..."
    tampilkan_progress 2 "Setting file"
    sudo chmod +x /usr/bin/akiratools.py /usr/bin/grup.py /usr/bin/spamup.py /usr/bin/spamori.py
